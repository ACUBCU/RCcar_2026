import threading, time, math, signal, sys, csv
...
print("======================================================")
print(" NEW CODE LOADED: CSV LOGGING (X, Y, CDS) ADDED       ")
print("======================================================")
...
# CSV 파일 초기화 (실행 시각 기준 파일명 생성 및 헤더 작성)
csv_filename = f"autocar_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
with open(csv_filename, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['X', 'Y', 'CDS'])
...
def hardware_loop():
    ...
    while not stop_event.is_set():
        ...
        if abs(estimated_velocity) > 0:
            ...
            # 주행 중일 때 실시간으로 CSV 파일에 데이터 누적 기록
            with open(csv_filename, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([round(robot_pose['x'], 3), round(robot_pose['y'], 3), current_cds])
